...
$lang    = in_array($_GET['lang'] ?? 'fr', ['fr','de','en']) ? ($_GET['lang'] ?? 'fr') : 'fr';
$aboutPg = $pdo->query("SELECT * FROM about_page WHERE id=1")->fetch();
...
<head>
  ...
  <meta name="description" content="Maison 1815 — Studio de production photo &amp; vidéo basé en Valais">
  <title>À PROPOS — MAISON 1815</title>
...
<body>
  <header id="navbar" class="navbar navbar--nav-only" role="banner">
    <nav aria-label="Navigation principale">
      <ul class="navbar__links">
        ...
        <li><a href="about.php"   class="nav-link" data-scramble data-en="ABOUT"   data-fr="À PROPOS" aria-current="page">ABOUT<span class="nav-link__line" aria-hidden="true"></span></a></li>
  ...
  <main>
    <section class="about-hero" aria-label="Introduction">
      <p class="about-hero__headline reveal">
        Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse varius, nunc at facilisis tincidunt, justo
        erat fermentum nunc, ut vulputate elit sapien at orci amet, consectetur adipiscing elit.
      </p>
      <dl class="about-info reveal">
        <div class="about-info__spacer" aria-hidden="true"></div>
        <div class="about-info__row">
          <dt class="about-info__label">contact</dt>
          <dd class="about-info__value">
            <a href="mailto:contact@maison1815.ch">contact@maison1815.ch</a><br>
            <a href="tel:+41270000000">+41 27 000 00 00</a>
          </dd>
        </div>
        <div class="about-info__row">
          <dt class="about-info__label">Office</dt>
          <dd class="about-info__value">maison1815 gmbh<br>platz 10<br>6039 root d4</dd>
        </div>
        <div class="about-info__row">
          <dt class="about-info__label">Socials</dt>
          <dd class="about-info__value">
    ...
    <section class="about-team" aria-label="Équipe">
      <div class="about-team__header">
        <span class="about-team__label">Équipe</span>
      </div>
      <ul class="team__list">
<?php foreach ($members as $m):
  $role = match($lang) { 'de' => $m['role_de'], 'en' => $m['role_en'], default => $m['role_fr'] };
  $photo = !empty($m['photo']) ? BASE_URL.'/uploads/'.$m['photo'] : 'https://placehold.co/400x500/0a0a0a/333333?text=+';
?>
        <li class="team__member reveal" data-img="<?= htmlspecialchars($photo,ENT_QUOTES,'UTF-8') ?>">
          <span class="team__member-name"><?= htmlspecialchars($m['first_name'].' '.$m['last_name'],ENT_QUOTES,'UTF-8') ?></span>
          <div class="team__member-meta">
            <span class="team__member-role"><?= htmlspecialchars($role,ENT_QUOTES,'UTF-8') ?></span>
            <?php if (!empty($m['email'])): ?>
            <span class="team__member-email"><a href="mailto:<?= htmlspecialchars($m['email'],ENT_QUOTES,'UTF-8') ?>"><?= htmlspecialchars($m['email'],ENT_QUOTES,'UTF-8') ?></a></span>
            <?php endif; ?>
          </div>
        </li>
<?php endforeach; ?>
      </ul>
      <img class="team__hover-img" src="" alt="" aria-hidden="true" draggable="false">
    </section>

    <div class="about-office" aria-hidden="true">
<?php if (!empty($aboutPg['image_path'])): ?>
      <img src="<?= htmlspecialchars(BASE_URL.$aboutPg['image_path'],ENT_QUOTES,'UTF-8') ?>" alt="Studio Maison 1815" loading="lazy" draggable="false">
<?php else: ?>
      <img src="https://placehold.co/1600x600/111111/222222?text=+" alt="Studio Maison 1815" loading="lazy" draggable="false">
<?php endif; ?>
    </div>
  </main>
